...
#Rscript ~/mosaic/production_code_06052018/strand_bias.R "$base".dp4.txt "$base"
#echo "done - see "$base".sb.txt"
## run parsefilters.py
echo "Running parsefilters.py"
#python ~/mosaic/ssc/gatk/parsefilters.gatk.py "$file" rfilt."$file" "$file" "$base".sb.txt> "$base".filtered.txt
python ~/mosaic/ssc/gatk/parsefilters.gatk.py "$file" rfilt."$file" "$file" > "$base".filtered.txt
...
## done message
echo "DONE ... see $base".filtered.txt""
python ~/mosaic/production_code_06052018/getfiltct.py "$base".filtered.txt
